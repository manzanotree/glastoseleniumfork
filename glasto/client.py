...
class Client(object):
    ...
    def __init__(
        ...
        if detach:
            self._service.options.add_experimental_option("detach", True)
    ...
    def establishconnection(
        ...
        while self.attempts < maxiterations:
            self.client = webdriver.Remote(
                self._service.url(), options=self._service.options
            )
            ...
            try:
                ...
                if self.verbose:
                    print(
                        "Page load Timeout Occured. Increasing timeout to {} and trying again....".format(
                            self.timeout
                        )
                    )
                self.client.quit()
    ...
    def clickbutton(self, substr):
        for button in self.client.find_elements("tag name", "button"):
...
class ScoutClient(Client):
    ...
    def getalllinks(self):
        elems = self.client.find_elements("xpath", "//a[@href]")
        return list(
            set(
                [
                    ...
                    and self.linkphrase in elem.get_attribute("href")
                ]
            )
        )
